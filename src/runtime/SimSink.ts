...
export class SimSink implements RobotSink {
  ...
  async exec(cmd: { command: string; params: any }): Promise<void> {
    ...
    switch (command) {
      ...
      case 'PLAY_TONE': {
        ...
        const ms = durMs(params) || 300;
        this.beep(hz, ms);
        if (params.wait) await this.sleep(ms);
        break;
      }
      ...
      case 'PLAY_RECORDING': {
        this.beep(660, 200);
        if (params.wait) await this.sleep(200);
        break;
      }
      case 'SET_VOLUME':
        this.volume = clamp((Number(params.value) || 0) / 100, 0, 1);
        break;
      case 'STOP_SOUNDS':
        this.commit({ buzzerHz: 0 }, true);
        break;
      case 'SET_ANALOG':
        this.state.sensors.analog = Number(params.value) || 0;
        this.commit({ sensors: { ...this.state.sensors } });
        break;
      case 'SET_DIGITAL':
        this.state.sensors.digital = params.value === 'HIGH' || params.value === 1 ? 1 : 0;
        this.commit({ sensors: { ...this.state.sensors } });
        break;
      case 'RESET_DISTANCE':
        this.odometry = 0;
  ...
  getSensorValue(getSensorDataJson: string): number | null {
    ...
    try {
      sensor = JSON.parse(getSensorDataJson)?.params?.sensor;
    } catch {
      return null;
    }
  ...
  private async animateDrive(params: any, mode: 'move' | 'turn' | 'steer'): Promise<void> {
    ...
    nextPorts[0] = leftPort;
    nextPorts[1] = rightPort;
    ...
    while (!this.stopRequested) {
      ...
      last = now;
      this.integrate(dt, mode, params, scale);
      await this.sleep(16);
    }
  ...
  private integrate(dt: number, mode: 'move' | 'turn' | 'steer', params: any, scale: number): void {
    ...
    this.commit({ x: p.x, y: p.y, headingDeg: p.heading, trail });
  }

  private async animateClaw(params: any): Promise<void> {
    this.stopRequested = false;
    const duration = durMs(params) || 600;
