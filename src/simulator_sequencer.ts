...
export class SimulatorSequencer {
  ...
  private executeActionCommand(command: any): Promise<void> {
    ...
    switch (commandName) {
      case 'SET_HEAD_POSITION':
        this.simulator.setHeadPosition(params.pitch, params.yaw);
        break;
      case 'SET_LED_COLOR':
        this.simulator.setLedColor(
          params.led_id,
          new THREE.Color(params.r / 255, params.g / 255, params.b / 255),
        );
        break;
      case 'DISPLAY_ICON':
        this.simulator.displayIcon(params.icon_name);
        break;
      case 'PLAY_INTERNAL_SOUND':
        this.simulator.playSound(params.sound_id);
        break;
    }
  ...
  private runTimedCommand(command: any, resolve: () => void): void {
    ...
    if (commandName === 'MOVE_TIMED' || commandName === 'TURN_TIMED') {
      ...
      this.simulator.playWheelAnimation(
        'L',
        isTurn
          ? direction === 'left'
            ? 'Backward'
    ...
    const tick = (currentTime: number) => {
      ...
      switch (commandName) {
        case 'MOVE_TIMED': {
          ...
          const moveDelta = new THREE.Vector2(forwardVector.x, forwardVector.z).multiplyScalar(
            distance,
          );
  ...
  private checkCircleRectangleCollision(
    ...
    const closestX = Math.max(
      ...
      Math.min(circlePos.x, rect.position.x + halfWidth),
    );
    const closestY = Math.max(
      rect.position.y - halfHeight,
      Math.min(circlePos.y, rect.position.y + halfHeight),
    );
  ...
  private getDistance(): number {
    ...
    const forwardVector = new THREE.Vector2(0, 1).rotateAround(
      new THREE.Vector2(),
      -this.simulator.robotModel.rotation.y,
    );
    const rightVector = new THREE.Vector2(1, 0).rotateAround(
      new THREE.Vector2(),
      -this.simulator.robotModel.rotation.y,
    );

    const centerRayOrigin = this.virtualPosition.clone();
    const leftRayOrigin = centerRayOrigin
      .clone()
      .add(rightVector.clone().multiplyScalar(-SENSOR_WIDTH_OFFSET));
    const rightRayOrigin = centerRayOrigin
      .clone()
      .add(rightVector.clone().multiplyScalar(SENSOR_WIDTH_OFFSET));
  ...
  public loadLevel(levelData: LevelData): void {
    ...
    if (levelData.environment.randomizeObstacles) {
      ...
      obstacles = obstacles.map((obs) => {
        const hasTag = randomizeConfig.tags.some((tag) => (obs as any)[tag] === true);
        if (hasTag) {
          const offsetY =
  ...
  public checkWinCondition(): boolean {
    ...
    const checkpointPos = new THREE.Vector2(
