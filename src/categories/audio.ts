...
defineOnce([
  // --- Microphone ---
  {
    ...
    "args0": [
      ...
      { "type": "field_dropdown", "name": "WAIT", "options": [["and continue when done", "false"], ["until done", "true"]] }
    ],
    "previousStatement": null, "nextStatement": null, "style": "audio_blocks", "inputsInline": true,
  },
  {
    ...
    "args0": [
      {
        "type": "field_dropdown", "name": "EFFECT", "options": [
          ["Short Beep", "short_beep"], ["Long Beep", "long_beep"], ["Chirp", "chirp"], ["Buzz", "buzz"], ["Ding", "ding"]
        ]
      },
      { "type": "field_dropdown", "name": "WAIT", "options": WAIT_OPTIONS }
    ],
  ...
  },
  {
    ...
    "previousStatement": null, "nextStatement": null, "style": "audio_blocks",
  },
  // --- Tempo ---
  {
    "type": "audio_set_bpm",
...
javascriptGenerator.forBlock['audio_record'] = function (block, gen) {
  return JSON.stringify({ command: astroidV2.commands.recordAudio, params: { slot: parseInt(block.getFieldValue('SLOT'), 10), secs: num(block, gen, 'DURATION'), wait: wait(block) } }) + ';';
};
javascriptGenerator.forBlock['audio_play_recording'] = function (block) {
  return JSON.stringify({ command: astroidV2.commands.playRecording, params: { slot: parseInt(block.getFieldValue('SLOT'), 10), wait: wait(block) } }) + ';';
};
javascriptGenerator.forBlock['audio_sound_effect'] = function (block) {
  return JSON.stringify({ command: astroidV2.commands.playSoundEffect, params: { effect: block.getFieldValue('EFFECT'), wait: wait(block) } }) + ';';
};
javascriptGenerator.forBlock['audio_play_tone_sec'] = function (block, gen) {
  return JSON.stringify({ command: astroidV2.commands.playTone, params: { note: block.getFieldValue('NOTE'), secs: num(block, gen, 'DURATION'), wait: wait(block) } }) + ';';
};
javascriptGenerator.forBlock['audio_play_tone_beat'] = function (block, gen) {
  return JSON.stringify({ command: astroidV2.commands.playTone, params: { note: block.getFieldValue('NOTE'), beats: num(block, gen, 'BEATS'), wait: wait(block) } }) + ';';
};
javascriptGenerator.forBlock['audio_set_volume'] = function (block) {
  return JSON.stringify({ command: astroidV2.commands.setVolume, params: { value: parseInt(block.getFieldValue('VALUE'), 10) } }) + ';';
};
...
javascriptGenerator.forBlock['audio_set_bpm'] = function (block) {
  return JSON.stringify({ command: astroidV2.commands.setBpm, params: { bpm: parseInt(block.getFieldValue('BPM'), 10) } }) + ';';
};
