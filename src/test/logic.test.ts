...
describe('Parity: Logic', () => {
  ...
  it('logic_operation handles AND and OR', async () => {
    expect(
      await moved({
        ...
        fields: { OP: 'AND' },
        inputs: { A: bool(true), B: bool(true) },
      }),
    ).toBe(true);
    expect(
      await moved({
        type: 'logic_operation',
        fields: { OP: 'AND' },
        inputs: { A: bool(true), B: bool(false) },
      }),
    ).toBe(false);
    expect(
      await moved({
        type: 'logic_operation',
        fields: { OP: 'OR' },
        inputs: { A: bool(false), B: bool(true) },
      }),
    ).toBe(true);
    expect(
      await moved({
        type: 'logic_operation',
        fields: { OP: 'OR' },
        inputs: { A: bool(false), B: bool(false) },
      }),
    ).toBe(false);
  });
  ...
  it('logic_compare compares two numbers', async () => {
    expect(
      await moved({ type: 'logic_compare', fields: { OP: 'GT' }, inputs: { A: 5, B: 3 } }),
    ).toBe(true);
    expect(
      await moved({ type: 'logic_compare', fields: { OP: 'GT' }, inputs: { A: 3, B: 5 } }),
    ).toBe(false);
    expect(
      await moved({ type: 'logic_compare', fields: { OP: 'EQ' }, inputs: { A: 4, B: 4 } }),
    ).toBe(true);
  });
});
